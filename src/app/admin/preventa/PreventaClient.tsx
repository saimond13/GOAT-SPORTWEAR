...
const STATUS_CONFIG = {
  ...
  deposit_paid: { label: "Seña pagada", color: "bg-[#556B5D]/10 text-[#556B5D]", icon: CheckCircle2 },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={label} className="bg-white border border-[#111111]/10 rounded-2xl p-4">
            <p className="text-[#B8B8B8] text-xs uppercase tracking-widest mb-1">{label}</p>
            <p className="text-[#111111] font-black text-xl">{value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-bold transition-colors ${
                filter === s
                  ...
                  : "bg-[#111111]/8 text-[#B8B8B8] hover:text-[#111111]"
              }`}
            >
        ...
        <button
          ...
          className="flex items-center gap-2 border border-[#111111]/10 text-[#2B2B2B] hover:text-[#111111] hover:border-[#111111]/30 font-bold px-4 py-2 rounded-xl text-sm transition-colors disabled:opacity-40"
        >
      ...
      {errorMsg && (
        <p className="text-red-500 text-sm bg-red-400/10 px-4 py-2.5 rounded-xl">{errorMsg}</p>
      )}

      {/* Table */}
      {filtered.length === 0 ? (
        <div className="bg-white border border-[#111111]/10 rounded-2xl p-12 text-center">
          <p className="text-[#B8B8B8] text-sm">Sin reservas en esta categoría.</p>
        </div>
      ) : (
        <div className="bg-white border border-[#111111]/10 rounded-2xl overflow-hidden overflow-x-auto">
          <table className="w-full min-w-[700px]">
            <thead>
              <tr className="border-b border-[#111111]/8">
                {["Cliente", "Drop", "Talle", "Seña", "Estado", "Fecha", ""].map((h) => (
                  <th key={h} className="text-left px-5 py-3 text-[10px] font-bold text-[#B8B8B8] uppercase tracking-widest">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {filtered.map((r) => {
                ...
                const Icon = cfg.icon;
                return (
                  <tr key={r.id} className="border-b border-[#111111]/8 hover:bg-[#111111]/[0.02]">
                    <td className="px-5 py-3">
                      <p className="text-[#111111] text-sm font-bold">{r.customer_name}</p>
                      <p className="text-[#B8B8B8] text-xs">{r.customer_phone}</p>
                      {r.customer_email && (
                        <p className="text-[#B8B8B8] text-xs">{r.customer_email}</p>
                      )}
                    </td>
                    <td className="px-5 py-3 text-[#2B2B2B] text-sm max-w-[160px]">
                      <p className="truncate">{r.campaign_title ?? "—"}</p>
                    </td>
                    <td className="px-5 py-3">
                      <span className="bg-[#111111]/8 text-[#2B2B2B] text-xs px-2 py-0.5 rounded-full">
                        {r.size} × {r.quantity}
                      </span>
                    </td>
                    <td className="px-5 py-3 text-[#111111] font-bold text-sm">
                    ...
                    <td className="px-5 py-3 text-[#B8B8B8] text-xs whitespace-nowrap">
                      {formatDate(r.created_at)}
                    </td>
                    <td className="px-5 py-3">
                      {loadingId === r.id ? (
                        <Loader2 className="w-4 h-4 animate-spin text-[#B8B8B8]" />
                      ) : (
                        <div className="flex flex-col gap-1">
                          {r.status === "pending" && (
                            <button
                              onClick={() => handleConfirm(r.id)}
                              className="flex items-center gap-1 text-[10px] text-[#556B5D] hover:text-[#4a5f52] font-black uppercase tracking-wide transition-colors"
                            >
                          ...
                          {r.status !== "cancelled" && (
                            <button
                              ...
                              className="text-[10px] text-red-400 hover:text-red-500 font-bold uppercase tracking-wide transition-colors"
                            >
                              Cancelar
                            </button>
                          )}
